...
class Trainer(object):
    ...
    def train(self, dataset):
        ...
        for data in tqdm(dataset):
            ...
            interaction = torch.LongTensor([data[2].astype(int)])

            comb = (
                protein1.to(device),
                adjacency1.to(device),
                protein2.to(device),
                adjacency2.to(device),
...
class Tester(object):
    ...
    def test(self, dataset):
        ...
        (
            ...
            ppv,
            npv,
        ) = calculate_performace(len(sampling), labels, y_true)
...
def calculate_performace(test_num, pred_y, labels):
    ...
    if (tp + fn) != 0 and (tn + fp) != 0:
        q9 = 1 - float(np.sqrt(2)) * np.sqrt(
            float(fn * fn) / ((tp + fn) * (tp + fn))
            + float(fp * fp) / ((tn + fp) * (tn + fp))
        )
...
import pandas as pd 
dir_input = "protein_fingerprints/"
# examples = np.load(dir_input + "train_examples.npy")
train_csv = pd.read_csv("ppi_data.csv")
examples = np.array(train_csv.values.tolist())
fingerprint_dict = load_pickle("protein_fingerprints/prot_fingerprint_dict.pickle")
...
for train, test in kfold.split(examples):
    ...
    with open(file_result, "w") as f:
        f.write(
            "Epoch \t Time(sec) \t Loss_train \t Accuracy \t Precision \t Recall \t Sensitivity \t Specificity \t MCC \t F1-score \t ROC_AUC \t AUC \t Q9 \t PPV \t NPV \t TP \t FP \t TN \t FN\n"
        )
